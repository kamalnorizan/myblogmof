...
<body>
    <div class="container">
        <div class="card mt-3">
            <div class="card-body">
                <h4 class="card-title">
                    <a href="{{ route('posts.show', ['post' => $post->uuid]) }}"> {{ $post->title }}</a> <small>~
                        {{ $post->user->name }} (<i>{{ $post->user->email }}</i>)</small>
                </h4>
                <p class="card-text">
                    {{ $post->content }}
                </p>
                ...
                <form method="POST" class="mb-5" name="commentForm" id="commentForm" action="{{ route('comment.store') }}"
                    >
                    @csrf
                ...
                @foreach ($post->comments as $comment)
                    <p><strong>{{ $comment->user->id . ') ' . $comment->user->name }}({{ $comment->user->posts->count() }})</strong>
                        {{ $comment->content }}</p>
                @endforeach
            </div>
        </div>
